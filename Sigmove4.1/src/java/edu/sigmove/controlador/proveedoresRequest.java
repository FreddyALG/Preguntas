...
public class proveedoresRequest implements Serializable {
    ...
    public void removerProveedor(Proveedores remProve) {
        ...
        try {
            ProveedoresFacadeLocal.removerProveedor(    remProve.getIdproveedor());
            listaProveed.remove(remProve);
            mensajeAlerta = "swal('Removido el usuario', '" + remProve.getNombre() + ' ' + remProve.getNumeroDocumento()+ "', 'success');";
        } catch (Exception e) {
